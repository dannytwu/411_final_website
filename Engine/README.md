...
To open editor. Within editor you have (in order) place tile, delete tile, fill tiles, select fill tiles, place sprite. You can use 1-5 to select these options too. Click which color you want from the top, or which sprite you want from the drop down sidebar on the right. To save a tilemap.dat, press s. To load the most recently saved version, pres l. To test play the current map, press the space bar.

dub run -c game -- maps/tilemapX.dat
This runs shootout.d using tilemapX.dat as the map (X being a variable for a valid tilemap.dat in the directory)
